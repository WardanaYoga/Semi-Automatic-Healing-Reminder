...
void gerakServoStep(Servo &servo, int &sudut, const char* key) {
  ...
  // LED merah nyala 10 detik jika servo baru sampai 180°
...
void resetSemuaServo() {
  ...
  // Matikan LED merah saat semua servo di-reset
  ledMerahAktif = false;
  digitalWrite(LED_MERAH, LOW);
}
...
void tampilPesan() {
  ...
  lcd.setCursor(0, 0); lcd.print("    Saatnya    ");
  lcd.setCursor(0, 1); lcd.print("  minum obat  ");
}
...
void handleSetJadwal() {
  ...
  // =====================================================
  // FIX UTAMA: Reset flag & override agar jadwal baru
  // langsung aktif tanpa perlu restart ESP32
  // =====================================================
  done1 = done2 = done3 = false;
  lastMinute     = -1;       // paksa re-evaluasi menit saat ini
  manualOverride = false;    // hapus override yang mungkin masih aktif

  // Feedback LCD konfirmasi jadwal tersimpan
  if (!lcdCustomAktif) {
    lcd.clear();
    lcd.setCursor(0, 0); lcd.print("  Jadwal Baru  ");
    char buf[17];
    sprintf(buf, " K%d -> %02d:%02d    ", k, h, m);
    lcd.setCursor(0, 1); lcd.print(buf);
    currentMode = MODE_ALARM;
    alarmStart  = millis();
  }
...
void setup() {
  ...
  // Kembalikan servo ke posisi terakhir
...
void loop() {
  server.handleClient();
  updateMP3();
  updateLED();   // timer LED non-blocking
  ...
  if (timeOk) {
    ...
    // Reset flag setiap ganti menit
    if (menit != lastMinute) {
      done1 = done2 = done3 = false;
      lastMinute = menit;
    }

    // Jadwal otomatis (hanya jika tidak ada manual override)
    if (!manualOverride) {
      if (jam == kloter1_hour && menit == kloter1_min && !done1) { jalankanKloter(1); done1 = true; }
      if (jam == kloter2_hour && menit == kloter2_min && !done2) { jalankanKloter(2); done2 = true; }
      if (jam == kloter3_hour && menit == kloter3_min && !done3) { jalankanKloter(3); done3 = true; }
    }
  }
